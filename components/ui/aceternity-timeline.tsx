...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  // Use ResizeObserver so height stays accurate across all viewport sizes
  useEffect(() => {
      ...
      >
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-background flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-neutral-800 border border-red-500/50 p-2" />
              </div>
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0 w-full bg-gradient-to-t from-red-500 via-red-400 to-transparent from-[0%] via-[10%] rounded-full"
          />
        </div>
      </div>
    </div>
